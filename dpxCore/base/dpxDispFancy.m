function dpxDispFancy(msg,symbol,nh,nv,textStyle,borderStyle)
    ...
    %   nv: Vertical thickness of border (n * numel(symbol))
    ...
    try
        ...
        if ~exist('nh','var') || isempty(nh)
            nh=2;
        end
        if ~exist('nv','var') || isempty(nv)
            nv=1;
        end
        if ~exist('textStyle','var')
            textStyle='*Text';
        end
        if ~exist('borderStyle','var') || isempty(borderStyle)
            borderStyle=[.7 .7 .7];
        end
