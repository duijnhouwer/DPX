...
function stim=settings2stim(C,physScr)
    try
        % C is a struct with condition parameters (from Settings-file)
        %
...
function checkConditionSettings(C)
    % Check the values of the condition
    errstr=[];
...
function checkStimSettings(S)
    % Check the values of the stim
    errstr=[];
    if ~isempty(errstr)
        jdPTBendExperiment;
        error(errstr);
