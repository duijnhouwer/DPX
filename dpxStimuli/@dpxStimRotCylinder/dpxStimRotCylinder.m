    ...
    methods (Access='protected')
        function myInit(S)
            ...
            elseif strcmpi(S.axis,'vertsphere')
                z=S.xCenterPx-S.wPx/2+S.wPx*S.RND.rand(1,S.nDots);
        ...
        function myDraw(S)
            ...
            for buffer=0:1
                ...
                if S.dispShiftMono
                    if sum(idx)>0
                        Screen('DrawDots', wPtr,S.XYZ([1 3],idx)+S.hordisp.lX00([1 3],idx),diam(idx),cols(:,idx), S.winCntrXYpx,1);
                        Screen('DrawDots', wPtr,S.XYZ([1 3],idx)+S.hordisp.rX00([1 3],idx),diam(idx),cols(:,idx), S.winCntrXYpx,1);
                    end
                elseif ~S.dispShiftMono
                    if sum(idx)>0
