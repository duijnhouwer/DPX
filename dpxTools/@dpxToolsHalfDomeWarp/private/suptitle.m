function  [axH,textH]=suptitle(str)
%SUPTITLE Puts a title above all subplots.
%	SUPTITLE('text') adds text to the top of the figure
%	above all subplots (a "super title"). Use this function
%	after all subplot commands.
% Returns handles to the super axes and the text.
...
% Ignore legend axes. BK July 2004.
out = strmatch('legend',get(h,'tag'));
if ~isempty(out)
    h(out)=[];
end
...
ha=axes('pos',[0 0 1 1],'visible','off','Tag','suptitle');
ht=text(.5,titleypos,str);set(ht,'horizontalalignment','center','fontsize',fs);
set(gcf,'nextplot',np);
% Commented out otherwise legend disappears
%axes(haold);
