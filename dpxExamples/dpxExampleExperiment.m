function dpxExampleExperiment(testscr)
    ...
    % dpxCoreExperiment object is made. The settings of scr can be viewed by
    % typing get(E.scr) and set by typing, for example,
    % set(E.scr,'distMm',1000) or E.scr.distMm=1000 to set the viewing distance
    % to a meter. Note that not all properties of E.scr that are displayed when
    % calling get(E.scr) can also be set using set, some properties are
    % read-only. A convenient way to set your scr properties, visualize, and
    ...
    % legibility; and (2) that an empty 'winRectPx' (i.e., []), triggers full
