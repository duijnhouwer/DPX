function jdMovieGrating(varargin)
    ...
    p.addOptional('pxWid',400,@(x)isnumeric(x) && ~rem(x,1) && x>0); % width of movie 
    p.addOptional('pxHei',300,@(x)isnumeric(x) && ~rem(x,1) && x>0); % height of movie
    p.addOptional('fadePx',-1,@isnumeric); % number of pixels to fade to mean lum, -1 no mask, 0, circular mask, 50 circular with 50 pixels linear RGB fade
    p.addOptional('cycPx',60,@(x)isnumeric(x) && x>0); % bar width
    p.addOptional('frN',250,@(x)isnumeric(x) && ~rem(x,1) && x>0); % number of frames
    p.addOptional('frHz',10,@(x)isnumeric(x) && x>0); % frame rate
    p.addOptional('aDeg',270,@isnumeric); % 
    p.addOptional('pxPerFr',7,@isnumeric); % pixels displacement per frame
