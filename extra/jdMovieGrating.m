function jdMovieGrating(varargin)
    ...
    p.addOptional('barPx',20,@(x)isnumeric(x) && x>0); % bar width
    p.addOptional('frN',60,@(x)isnumeric(x) && ~rem(x,1) && x>0); % number of frames
    p.addOptional('frHz',30,@(x)isnumeric(x) && x>0); % frame rate
    p.addOptional('dXdFrPx',1.5,@isnumeric); % pixels displacement per frame
