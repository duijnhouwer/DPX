...
function [E,windowPtr]=prepareExperiment
    ...
    [E.conditions,E.nBlocks,E.instruction,E.outputfolder,hardware]=eval(settingsFileName);
...
function [windowPtr,physScr]=openStimWindow(varargin)
    ...
    p.addParamValue('screen', struct('widHeiMm',[],'distMm',500,'gamma',1,'stereoMode','mono','window',[]),@isstruct);
    p.parse(varargin{:});
    %
    if strcmpi(p.Results.hardware.stereoMode,'mono')
        physScr.stereoCode=0;
    elseif strcmpi(p.Results.hardware.stereoMode,'mirror')
        physScr.stereoCode=4;
    else
        jdPTBendExperiment
        keyboard;
    end
    ...
    if isempty(p.Results.screen.widHeiMm)
        [physScr.widMm, physScr.heiMm]=Screen('DisplaySize',physScr.scrNr);
    else
        physScr.widMm = p.Results.screen.widHeiMm(1); %406;
        physScr.heiMm = p.Results.screen.widHeiMm(2); %305;
    end
    physScr.whiteIdx = WhiteIndex(physScr.scrNr);
    physScr.blackIdx = BlackIndex(physScr.scrNr);
    physScr.distMm = p.Results.screen.distMm;
    ...
    [windowPtr, physScr.winRect] = Screen('OpenWindow',physScr.scrNr,[0 0 255],window,[],2,physScr.stereoCode); %[0 0 physScr.widPx physScr.heiPx],[],2);
    physScr.gamma = p.Results.screen.gamma;
